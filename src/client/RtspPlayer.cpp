//
//  RtspPlayer.cpp
//  toolForTest
//
//  Created by cx on 2018/9/6.
//  Copyright © 2018年 cx. All rights reserved.
//

#include "rtsplib/client/RtspPlayer.h"

#ifdef _WIN32
#define NOMINMAX
#include <WS2tcpip.h>
#include <WinSock2.h>
#include <Windows.h>
#include <io.h>
#elif __unix__
#include <netinet/in.h>
#include <sys/socket.h>
#include <unistd.h>
//#include <netdb.h>
#endif

#include <algorithm>
#include <cstdio>
#include <cstdlib>
#include <cstring>
#include <spdlog/spdlog.h>
#include <sys/types.h>
#include <utility>

#define log(tag, fmt, ...)                                                     \
  do {                                                                         \
    printf("[%s] > ", tag.c_str());                                            \
    printf(fmt, ##__VA_ARGS__);                                                \
    printf("\n");                                                              \
  } while (0)

#define SetNextState(x) _PlayState = x;

namespace RK {

std::mutex RtspPlayer::_portMutex;
std::vector<int> RtspPlayer::_known_used_Ports;

RtspPlayer::RtspPlayer(RecvBufferFn recv_cb, const std::string &name,
                       int rtp_port, int rtcp_port)
    : recv_cb(std::move(recv_cb)), _rtp_port(rtp_port), _rtcp_port(rtcp_port) {
  TAG = "RTSP_" + name;

  _Terminated = false;
  _NetWorked = false;
  _PlayState = RtspIdle;

#ifdef _WIN32
  WSADATA wsaData;
  WSAStartup(MAKEWORD(2, 0), &wsaData);
#endif
}

#ifdef _WIN32
RtspPlayer::~RtspPlayer() { WSACleanup(); }
#else
RtspPlayer::~RtspPlayer() = default;
#endif

ImgProps RtspPlayer::GetImageProperties() { return _ImgProps; }

bool RtspPlayer::getIPFromUrl(const std::string &url, char *ip, unsigned short *port) {
  unsigned int dstip[4] = {0};
  int dstport = 0;
  int field = sscanf(url.c_str(), "rtsp://%d.%d.%d.%d:%d", &dstip[0], &dstip[1],
                     &dstip[2], &dstip[3], &dstport);
  //        if (field < 4) {
  //            spdlog::debug("getIPFromUrl: failed to get ip from url");
  //            return false;
  //        } else if (field == 4) {
  if (field == 4) {
    sprintf(ip, "%d.%d.%d.%d", dstip[0], dstip[1], dstip[2], dstip[3]);
    *port = dstport = 554;
  } else if (field == 5) {
    sprintf(ip, "%d.%d.%d.%d", dstip[0], dstip[1], dstip[2], dstip[3]);
    *port = dstport;
  } else {
    spdlog::debug("getIPFromUrl: failed to get ip from url");
    return false;
  }

  return true;
}

bool RtspPlayer::NetworkInit(const char *ip, const short port) {
  _RtspSocket = ::socket(AF_INET, SOCK_STREAM, 0);
  if (_RtspSocket < 0) {
    spdlog::debug("NetworkInit: network init failed");
    return false;
  }

  _Eventfd = _RtspSocket > _Eventfd
                 ? _RtspSocket
                 : _Eventfd; // std::max(_RtspSocket, _Eventfd);

  struct sockaddr_in serverAddr {};
  serverAddr.sin_family = AF_INET;
  serverAddr.sin_port = htons(port);
  serverAddr.sin_addr.s_addr = inet_addr(ip);

  if (::connect(_RtspSocket, (struct sockaddr *)&serverAddr,
                (socklen_t)sizeof(serverAddr)) == 0) {
    spdlog::debug("NetworkInit: sync connect success");
    _NetWorked = true;
  } else if (errno == EINPROGRESS) {
    spdlog::debug("NetworkInit: async connecting...");
  } else {
    spdlog::debug("NetworkInit: invalid connect");
    return false;
  }

  return true;
}

bool RtspPlayer::RTPSocketInit(int videoPort, int audioPort) {
  if (videoPort) {
    _RtpVideoSocket = ::socket(AF_INET, SOCK_DGRAM, 0);
    if (_RtpVideoSocket < 0) {
      spdlog::debug("RTPSocketInit: rtp video socket init failed");
      return false;
    }
    _Eventfd = _RtpVideoSocket > _Eventfd
                   ? _RtpVideoSocket
                   : _Eventfd; // std::max(_RtpVideoSocket, _Eventfd);

    size_t recvBufSize = 731504439992;
    if (::setsockopt(_RtpVideoSocket, SOL_SOCKET, SO_RCVBUF,
                     (const char *)&recvBufSize,
                     (int)sizeof(recvBufSize)) == -1) {
      spdlog::debug(
          "RTPSocketInit: failed to set rtsp video socket receive buffer size");
    }

#ifdef _WIN32
    u_long ul = true;
    if (ioctlsocket(_RtpVideoSocket, FIONBIO, &ul)) {
      spdlog::debug("RTPSocketInit: failed to set rtp video socket non block");
      ::closesocket(_RtpVideoSocket);
      return false;
    }
#elif __unix__
    int ul = true;
    if (::ioctl(_RtpVideoSocket, FIONBIO, &ul) < 0) {
      spdlog::debug("RTPSocketInit: failed to set rtp video socket non block");
      ::close(_RtpVideoSocket);
      return false;
    }
#endif
    _RtpVideoAddr.sin_family = AF_INET;
    _RtpVideoAddr.sin_addr.s_addr = INADDR_ANY;
    _RtpVideoAddr.sin_port = htons(videoPort);

    if (::bind(_RtpVideoSocket, (const struct sockaddr *)&_RtpVideoAddr,
               (socklen_t)sizeof(_RtpVideoAddr)) < 0) {
      spdlog::debug(
          "RTPSocketInit: failed to bind rtp video socket error {}, {}", errno,
          strerror(errno));
      return false;
    }

    return true;
  }

  return false;
}

void RtspPlayer::EventInit() {
  FD_ZERO(&_readfd);
  FD_ZERO(&_writefd);
  FD_ZERO(&_errorfd);

  FD_SET(_RtspSocket, &_readfd);
  FD_SET(_RtspSocket, &_writefd);
  FD_SET(_RtspSocket, &_errorfd);
}

std::vector<std::string> RtspPlayer::GetSDPFromMessage(const char *buffer,
                                                       size_t length,
                                                       const char *pattern) {
  char *tempBuffer = (char *)malloc(length + 1);
  strcpy(tempBuffer, buffer);

  std::vector<std::string> rvector;
  char *tmpStr = strtok(tempBuffer, pattern);

  while (tmpStr != nullptr) {
    rvector.emplace_back(tmpStr);
    tmpStr = strtok(nullptr, pattern);
  }

  free(tempBuffer);

  return rvector;
}

void RtspPlayer::SendDescribe(const std::string &url) const {
  char buf[1024];
  sprintf(buf,
          "DESCRIBE %s RTSP/1.0\r\n"
          "Accept: application/sdp\r\n"
          "CSeq: %d\r\n"
          "User-Agent: Lavf58.12.100\r\n"
          "\r\n",
          url.c_str(), RTSPDESCRIBE);

  ::send(_RtspSocket, buf, strlen(buf), 0);
}

void RtspPlayer::HandleDescribe(const char *buf, ssize_t bufsize) {

  std::vector<std::string> rvector = GetSDPFromMessage(buf, bufsize, "\r\n");
  std::string sdp;
  for (const auto &substr : rvector) {
    if (strstr(substr.c_str(), "Session:")) {
      ::sscanf(substr.c_str(), "Session:%ld", &_RtspSessionID);
    } else if (strchr(substr.c_str(), '=')) {
      sdp.append(substr);
      sdp.append("\n");
    }
  }
  if (sdp.empty()) {
    spdlog::debug(TAG, "received empty sdp message");
    return;
  }

  _SdpParser = sdp_parse(sdp.c_str());

  // Get image properties from describe string
  for (int i = 0; i < _SdpParser->medias_count; i++) {
    for (int j = 0; j < _SdpParser->medias[i].attributes_count; j++) {
      char *indexFound = nullptr;
      indexFound = strstr(_SdpParser->medias[i].attributes[j], "x-dimensions:");
      if (indexFound) {
        indexFound += 13; // length of "x-dimensions:"
        char *values = strtok(indexFound, ",");
        if (values)
          _ImgProps.width = atoi(values);
        values = strtok(nullptr, ",");
        if (values)
          _ImgProps.height = atoi(values);
        values = strtok(nullptr, ",");
        if (values)
          _ImgProps.bytesPerPixel = atoi(values);
      }
    }
  }
  if (_ImgProps.width > 0 && _ImgProps.height > 0 &&
      _ImgProps.bytesPerPixel > 0 && imgPropRdy_cb != nullptr) {
    imgPropRdy_cb(_ImgProps.width, _ImgProps.height, _ImgProps.bytesPerPixel);
  }
}

void RtspPlayer::RtspSetup(const std::string &url, int track, int CSeq,
                           char *proto, short rtp_port, short rtcp_port) const {
  char buf[1024];
  sprintf(buf,
          "SETUP %s/trackID=%d RTSP/1.0\r\n"
          "CSeq: %d\r\n"
          "User-Agent: Lavf58.12.100\r\n"
          "Transport: %s;unicast;client_port=%d-%d\r\n"
          "\r\n",
          url.c_str(), track, CSeq, proto, rtp_port, rtcp_port);

  ::send(_RtspSocket, buf, strlen(buf), 0);
}

void RtspPlayer::SendVideoSetup() {

  int i = 0, j = 0;
  int videoTrackID = 0;
  for (i = 0; i < _SdpParser->medias_count; i++) {
    if (strcmp(_SdpParser->medias[i].info.type, "video") == 0) {
      for (j = 0; j < _SdpParser->medias[i].attributes_count; j++) {
        if (strstr(_SdpParser->medias[i].attributes[j], "trackID")) {
          ::sscanf(_SdpParser->medias[i].attributes[j], "control:trackID=%d",
                   &videoTrackID);
        }
      }

      {
        std::lock_guard<std::mutex> guard(_portMutex);
        _video_rtp_port = _rtp_port;
        while (!PortIsOpen(_video_rtp_port))
          _video_rtp_port++;

        _video_rtcp_port = _rtcp_port;
        while (!PortIsOpen(_video_rtcp_port))
          _video_rtcp_port++;

        _known_used_Ports.push_back(_video_rtp_port);
        _known_used_Ports.push_back(_video_rtcp_port);
        RtspSetup(_rtspurl, videoTrackID, RTSPVIDEO_SETUP,
                  _SdpParser->medias[i].info.proto, _video_rtp_port,
                  _video_rtcp_port);
      }
    }
  }
}

bool RtspPlayer::HandleVideoSetup(const char *buf, ssize_t bufsize) {
  int remote_port = 0;
  int remote_rtcp_port = 0;

  if (strstr(buf, "server_port=")) {
    ::sscanf(strstr(buf, "server_port="), "server_port=%d-%d", &remote_port,
             &remote_rtcp_port);
  }

  if (!RTPSocketInit(_video_rtp_port, 0)) {
    spdlog::debug("HandleVideoSetup: rtp socket init failed");
    return false;
  }

  struct sockaddr_in remoteAddr {};
  remoteAddr.sin_family = AF_INET;
  remoteAddr.sin_port = htons(remote_port);
  remoteAddr.sin_addr.s_addr = inet_addr(_rtspip);

  const unsigned char natpacket[] = {0x80, 0x00, 0x00, 0x00, 0x00, 0x00,
                                     0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  ::sendto(_RtpVideoSocket, (const char *)natpacket, sizeof(natpacket), 0,
           (const struct sockaddr *)&remoteAddr, (socklen_t)sizeof(remoteAddr));

  return true;
}

void RtspPlayer::SendPlay(const std::string &url) const {
  char buf[1024];
  sprintf(buf,
          "PLAY %s RTSP/1.0\r\n"
          "CSeq: %u\r\n"
          "Session: %ld\r\n"
          "Range: npt=0.000-\r\n" // Range
          "User-Agent: Lavf58.12.100\r\n"
          "\r\n",
          url.c_str(), RTSPPLAY, _RtspSessionID);

  ::send(_RtspSocket, buf, strlen(buf), 0);
}

bool RtspPlayer::HandleRtspMsg(const char *buf, ssize_t bufsize) {
  int MsgType = 0;
  if (::sscanf(buf, "%*[^C]CSeq:%d", &MsgType) != 1) {
    spdlog::debug(TAG, "invalid rtsp message");
    return false;
  }

  switch (MsgType) {
  case RTSPOPTIONS:

    break;
  case RTSPDESCRIBE:
    HandleDescribe(buf, bufsize);
    SetNextState(RtspSendVideoSetup);
    break;
  case RTSPVIDEO_SETUP:
    if (HandleVideoSetup(buf, bufsize)) {
      SetNextState(RtspSendPlay);
    }
    break;

  case RTSPAUDIO_SETUP:
    break;

  case RTSPPLAY:
    break;

  default:
    spdlog::debug("unknown rtsp message");
    break;
  }

  return true;
}

void RtspPlayer::HandleRtspState() {

  switch (_PlayState.load()) {
  case RtspSendOptions:
    spdlog::debug("rtsp send options");
    break;
  case RtspHandleOptions:
    spdlog::debug("rtsp handle options");
    break;
  case RtspSendDescribe:
    spdlog::debug("rtsp send describe");
    SendDescribe(_rtspurl);
    break;
  case RtspHandleDescribe:
    spdlog::debug("rtsp handle describe");
    break;
  case RtspSendVideoSetup:
    spdlog::debug("rtsp send video setup");
    SendVideoSetup();
    break;
  case RtspHandleVideoSetup:
    spdlog::debug("rtsp handle video setup");
    break;
  case RtspSendAudioSetup:
    spdlog::debug("rtsp send audio setup");
    break;
  case RtspHandleAudioSetup:
    spdlog::debug("rtsp handle audio setup");
    break;
  case RtspSendPlay:
    spdlog::debug("rtsp send play");
    SendPlay(_rtspurl);
    break;
  case RtspHandlePlay:
    spdlog::debug("rtsp handle play");
    break;
  case RtspSendPause:
    spdlog::debug("rtsp send pause");
    break;
  case RtspHandlePause:
    spdlog::debug("rtsp handle pause");
    break;
  case RtspIdle:
    break;
  default:
    spdlog::debug("unknown rtsp state");
    break;
  }

  SetNextState(RtspIdle);
}

// void RtspPlayer::HandleRtpMsg(const char *buf, ssize_t bufsize) {
//     char header[] = {0, 0, 0, 1};
//     struct Nalu nalu = *(struct Nalu *)(buf + RTP_OFFSET);

//     if (!fp) {
//         fp = ::fopen("test.h264", "w+");
//         if (!fp) {
//             spdlog::debug("failed to oepen test.h264");
//             return;
//         }
//     }

//     if (nalu.type >= 0 && nalu.type < 24) { //one nalu
//         ::fwrite(header, 4, 1, fp);
//         ::fwrite(buf + RTP_OFFSET, bufsize - RTP_OFFSET, 1, fp);
//         ::fflush(fp);
//     } else if (nalu.type == 28) { //fu-a slice
//         struct FU fu;
//         char in = buf[RTP_OFFSET + 1];
//         fu.S = in >> 7;
//         fu.E = (in >> 6) & 0x01;
//         fu.R = (in >> 5) & 0x01;
//         fu.type = in & 0x1f;

//         if (fu.S == 1) {
//             char naluType = nalu.forbidden_zero_bit << 7 | nalu.nal_ref_idc
//             << 5 | fu.type;
//             ::fwrite(header, 4, 1, fp);
//             ::fwrite(&naluType, 1, 1, fp);
//             ::fwrite(buf + FU_OFFSET, bufsize - FU_OFFSET, 1, fp);
//             ::fflush(fp);
//         } else if (fu.E == 1) {
//             ::fwrite(buf + FU_OFFSET, bufsize - FU_OFFSET, 1, fp);
//             ::fflush(fp);
//         } else {
//             ::fwrite(buf + FU_OFFSET, bufsize - FU_OFFSET, 1, fp);
//             ::fflush(fp);
//         }
//     }
// }

bool RtspPlayer::Play(const std::string &url) {

  _PlayThreadPtr = std::make_shared<std::thread>([&, url] {
    char ip[256] = {0};
    unsigned short port = 0;
    _rtspurl = url;

    if (!getIPFromUrl(url, ip, &port)) {
      spdlog::debug("get ip and port failed");
      return false;
    }
    ::memcpy(_rtspip, ip, sizeof(ip));

    spdlog::debug("async connecting...");
#ifdef _WIN32
    while (!PlayLoop(ip, port)) {
      Sleep(1000);
    }
#elif __unix__
    while (!PlayLoop(ip, port)) {
      usleep(100000);
    }
#endif
  });

  return true;
}

bool RtspPlayer::PlayLoop(const char *ip, unsigned short port) {
  uint8_t recvbuf[2048];

  while (!NetworkInit(ip, port) && !_Terminated) {
#ifdef _WIN32
    Sleep(1000);
#elif __unix__
    usleep(100000);
#endif
  }

  EventInit();

  struct timeval tv {
  }, tv1{};
  tv.tv_sec = 2;
  tv.tv_usec = 0;

  while (!_Terminated) {
    FD_ZERO(&_readfd);
    FD_ZERO(&_errorfd);
    FD_SET(_RtspSocket, &_readfd);
    FD_SET(_RtspSocket, &_errorfd);

    // rtp video socket has connected
    if (_RtpVideoSocket) {
      FD_SET(_RtpVideoSocket, &_readfd);
    }

    memcpy(&tv1, &tv, sizeof(tv));
    int r = ::select(_Eventfd + 1, &_readfd, &_writefd, &_errorfd, &tv1);

    if (r < 0) {
      spdlog::debug("PlayLoop: event error...");
      break;
    } else if (r == 0) {
      // Timeout
      continue;
    } else {
      if (FD_ISSET(_RtspSocket, &_readfd)) {

        ::memset(recvbuf, 0, sizeof(recvbuf));
        ssize_t recvbytes =
            ::recv(_RtspSocket, (char *)recvbuf, sizeof(recvbuf), 0);

        if (recvbytes <= 0) {
          spdlog::debug("PlayLoop: async reconnecting...");
#if (_WIN32)
          closesocket(_RtspSocket);
          closesocket(_RtpVideoSocket);
#else
          close(_RtspSocket);
          close(_RtpVideoSocket);
#endif

          return false;
        } else {
          if (!HandleRtspMsg((char *)recvbuf, recvbytes)) {
            spdlog::debug("failed to handle rtsp msg");
          }
        }
      }

      if (FD_ISSET(_RtpVideoSocket, &_readfd)) {

        socklen_t socklen = sizeof(_RtpVideoAddr);
        ::memset(recvbuf, 0, sizeof(recvbuf));

        ssize_t recvbytes =
            ::recvfrom(_RtpVideoSocket, (char *)recvbuf, sizeof(recvbuf), 0,
                       (struct sockaddr *)&_RtpVideoAddr, &socklen);
        spdlog::debug("recv rtp video packet {} bytes", recvbytes);

        if (recvbytes > 0 && recv_cb != nullptr) {
          recv_cb(recvbuf, recvbytes);
        }
      }

      if (FD_ISSET(_RtspSocket, &_writefd)) {
        spdlog::debug("async connect success");
        SetNextState(RtspSendDescribe);
        FD_CLR(_RtspSocket, &_writefd);
      }

      if (FD_ISSET(_RtspSocket, &_errorfd)) {
        spdlog::debug("event error occur");
        break;
      }
    }

    HandleRtspState();
  }

  return true;
}

void RtspPlayer::Stop() {
  _Terminated = true;
  _PlayThreadPtr->join();
  std::remove(_known_used_Ports.begin(), _known_used_Ports.end(), _video_rtp_port);
  std::remove(_known_used_Ports.begin(), _known_used_Ports.end(), _video_rtcp_port);
  spdlog::debug("client stopped");
}

bool RtspPlayer::PortIsOpen(int port) {
  for (int _known_used_Port : _known_used_Ports) {
    if (_known_used_Port == port)
      return false;
  }

  int socket = ::socket(AF_INET, SOCK_DGRAM, 0);
  if (socket < 0) {
    return false;
  }

  struct sockaddr_in addr {};
  addr.sin_family = AF_INET;
  addr.sin_addr.s_addr = 0;
  addr.sin_addr.s_addr = INADDR_ANY;
  addr.sin_port = htons(port);

  if (::bind(socket, (const struct sockaddr *)&addr,
             sizeof(struct sockaddr_in)) < 0) {
    return false;
  }

#if (_WIN32)
  closesocket(socket);
#else
  close(socket);
#endif

  return true;
}
} // namespace RK
