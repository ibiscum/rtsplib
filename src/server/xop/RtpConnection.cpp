...
bool RtpConnection::setupRtpOverUdp(MediaChannelId_t channelId, uint16_t rtpPort,
  ...
  for (int n = 0; n <= 10; n++) {
    ...
    if (!SocketUtil::bind(_rtpfd[channelId], "0.0.0.0",
                          _localRtpPort[channelId])) {
      SocketUtil::close(_rtpfd[channelId]);
      continue;
    }

    _rtcpfd[channelId] = ::socket(AF_INET, SOCK_DGRAM, 0);
    if (!SocketUtil::bind(_rtcpfd[channelId], "0.0.0.0",
                          _localRtcpPort[channelId])) {
      SocketUtil::close(_rtpfd[channelId]);
  ...
  _peerRtpAddr[channelId].sin_port =
      htons(_mediaChannelInfo[channelId].rtpPort);

  _peerRtcpAddr[channelId].sin_family = AF_INET;
  _peerRtcpAddr[channelId].sin_addr.s_addr = _peerAddr.sin_addr.s_addr;
  _peerRtcpAddr[channelId].sin_port =
      htons(_mediaChannelInfo[channelId].rtcpPort);
...
void RtpConnection::play() {
  for (auto &chn : _mediaChannelInfo) {
    if (chn.isSetup) {
      chn.isPlay = true;
    }
  }
...
string RtpConnection::getRtpInfo(const std::string &rtspUrl) {
  ...
  auto timePoint = chrono::time_point_cast<chrono::milliseconds>(
      chrono::steady_clock::now());
  auto ts = timePoint.time_since_epoch().count();
  for (int chn = 0; chn < MAX_MEDIA_CHANNEL; chn++) {
    auto rtpTime = (uint32_t)(ts * _mediaChannelInfo[chn].clockRate / 1000);
    if (_mediaChannelInfo[chn].isSetup) {
      if (numChannel != 0)
        snprintf(buf + strlen(buf), sizeof(buf) - strlen(buf), ",");

      snprintf(buf + strlen(buf), sizeof(buf) - strlen(buf),
               "url=%s/track%d;seq=0;rtptime=%u", rtspUrl.c_str(), chn,
               rtpTime);
      numChannel++;
    }
  }
...
void RtpConnection::setRtpHeader(MediaChannelId_t channelId, const RtpPacket& pkt) {
  if ((_mediaChannelInfo[channelId].isPlay ||
    ...
    _mediaChannelInfo[channelId].rtpHeader.seq =
        htons(_mediaChannelInfo[channelId].packetSeq++);
    memcpy(pkt.data.get() + 4, &_mediaChannelInfo[channelId].rtpHeader,
           RTP_HEADER_SIZE);
  }
}
...
int RtpConnection::sendRtpPacket(MediaChannelId_t channelId, const RtpPacket& pkt, bool isGOPCache) {
  ...
  bool ret =
      _rtspConnection->_pTaskScheduler->addTriggerEvent([this, channelId, pkt] {
        ...
        if ((_mediaChannelInfo[channelId].isPlay ||
          ...
          } else // if(_transportMode == RTP_OVER_UDP ||
                 // _transportMode==RTP_OVER_MULTICAST)
          {
...
int RtpConnection::sendRtpOverUdp(MediaChannelId_t channelId, const RtpPacket& pkt) {
  ...
  int ret =
      sendto(_rtpfd[channelId], (const char *)pkt.data.get() + 4, pkt.size - 4,
             0, (struct sockaddr *)&(_peerRtpAddr[channelId]),
