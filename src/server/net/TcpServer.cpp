...
TcpServer::TcpServer(EventLoop *eventLoop, const std::string& ip, uint16_t port)
    : _eventLoop(eventLoop), _acceptor(new Acceptor(eventLoop, ip, port)) {
  _ip = ip;
  _port = port;
  ...
  _acceptor->setNewConnectionCallback([this](SOCKET sockfd) {
    ...
    if (tcpConn) {
      ...
      tcpConn->setDisconnectCallback([this](const TcpConnection::Ptr& conn) {
        auto taskScheduler = conn->getTaskScheduler();
        int sockfd = conn->fd();
        if (!taskScheduler->addTriggerEvent(
                [this, sockfd] { this->removeConnection(sockfd); })) {
          taskScheduler->addTimer(
              [this, sockfd]() {
...
int TcpServer::start() { return _acceptor->listen(); }

TcpConnection::Ptr TcpServer::newConnection(SOCKET sockfd) {
  return std::make_shared<TcpConnection>(_eventLoop->getTaskScheduler().get(),
                                         sockfd);
}

void TcpServer::addConnection(SOCKET sockfd, const TcpConnection::Ptr& tcpConn) {
