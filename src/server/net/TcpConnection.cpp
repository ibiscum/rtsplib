...
TcpConnection::TcpConnection(TaskScheduler *taskScheduler, int sockfd)
    : _taskScheduler(taskScheduler), _readBufferPtr(new BufferReader),
      _writeBufferPtr(new BufferWriter(500)), _channelPtr(new Channel(sockfd)) {
  _isClosed = false;
...
void TcpConnection::send(std::shared_ptr<char> data, uint32_t size) {
  ...
  _writeBufferPtr->append(std::move(data), size);
  this->handleWrite();
  // return;
}
...
void TcpConnection::handleRead() {
  ...
  int retbuf = _readBufferPtr->readFd(_channelPtr->fd());
  if (retbuf <= 0) {
    this->handleClose();
    return;
  }
...
void TcpConnection::handleError() { this->handleClose(); }
