...
void xop::Free(void *ptr) { return MemoryManager::Instance().Free(ptr); }
...
void MemoryPool::Init(uint32_t size, uint32_t n) {
  ...
  for (uint32_t i = 1; i < _numBlocks; i++) {
    auto *next = (MemoryBlock *)(_memory + (i * (_blockSize + sizeof(MemoryBlock))));
    next->_blockId = i + 1;
    next->_pool = this;
    next->_next = nullptr;
...
void *MemoryManager::Alloc(uint32_t size) {
  for (int n = 0; n < kMaxMemoryPool; n++) {
    if (size <= _memoryPools[n].BlockSize()) {
