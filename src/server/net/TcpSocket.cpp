...
TcpSocket::TcpSocket(SOCKET sockfd) : _sockfd(sockfd) {
#ifdef _WIN32
  WSADATA wsaData;
...
bool TcpSocket::listen(int backlog) const {
  if (::listen(_sockfd, backlog) == SOCKET_ERROR) {
    spdlog::debug("<socket={}> listen failed.", _sockfd);
    return false;
  }
...
bool TcpSocket::connect(const std::string& ip, uint16_t port, int timeout) const {
  if (!SocketUtil::connect(_sockfd, ip, port, timeout)) {
    spdlog::debug("<socket={}> connect failed.", _sockfd);
    return false;
  }
